...
|---|---|
| `mcp/*.rs` | 17 tool implementations, one `#[tool_router]` per module, split by domain below |
| `mcp/server.rs` | Server state, sandbox registry, conclusion persistence, and helpers shared by the tool modules |
| `mcp/project.rs`, `mcp/analysis.rs`, `mcp/coverage.rs`, `mcp/annotations.rs`, `mcp/sandbox.rs`, `mcp/conclusions.rs` | The tool handlers themselves |
| `mcp/concurrency.rs` | The one handler that never reaches Frama-C: Level-0 concurrency screening reads the source as text, so its evidence is syntax and it reports candidates only |
