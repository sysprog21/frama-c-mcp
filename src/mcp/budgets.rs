...
pub(crate) const WP_PROOF_BUDGET: Duration = Duration::from_secs(600);
...
pub(crate) const WP_DRAIN_BUDGET: Duration = WP_PROOF_BUDGET;
...
pub(crate) const EVA_COMPUTE_BUDGET: Duration = Duration::from_secs(600);

/// Ceiling on kernel.ast.compute, which reparses the project.
pub(crate) const AST_COMPUTE_BUDGET: Duration = Duration::from_secs(120);
...
/// never does.
pub(crate) const PLUGIN_EXEC_BUDGET: Duration = Duration::from_secs(30);
...
pub(crate) const PROBE_CONNECT_BUDGET: Duration = Duration::from_secs(5);
