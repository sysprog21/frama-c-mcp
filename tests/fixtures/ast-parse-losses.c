 ...
 * inline-assembly memory clobber whose real effects Frama-C cannot see, and two
 * dropped declarations. Counted as 2 clobber sites and 2 distinct attribute
 * names.
 */
int clobber_one(int x)
{
    __asm__ volatile("" ::: "memory");
    return x;
}
int clobber_two(int x)
{
    __asm__ volatile("" ::: "memory");
    return x;
}
